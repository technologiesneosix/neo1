  ...
  try {
    ...
    try {
      ...
      logResult(
        ...
        false,
        "Should have failed without required fields",
      );
    } catch (error) {
      if (error.errors) {
        logResult(
    ...
    try {
      ...
      logResult(
        "Unique Index",
        false,
        "Should have failed with duplicate slug",
      );
      ...
      if (error.code === 11000) {
        logResult(
          "Unique Index",
          true,
          "Correctly enforces unique constraint on slug",
        );
        await Technology.deleteMany({ slug: "test-tech" });
      } else {
    ...
    try {
      ...
      logResult(
        "Enum Validation",
        false,
        "Should have failed with invalid enum",
      );
    } catch (error) {
    ...
    try {
      ...
      const defaultsMatch = tech.status === "draft" && tech.displayOrder === 0;
    ...
    try {
      ...
      const hasPopulated =
        populatedService.technologies[0].name === "Relation Test";
      logResult(
    ...
    try {
      ...
      logResult(
        ...
        hasNoPassword && hasNoRefreshToken,
        "Password and refreshToken excluded from JSON",
      );
    ...
    try {
      ...
      logResult(
        "Email Validation",
        false,
        "Should have failed with invalid email",
      );
    } catch (error) {
    ...
    try {
      ...
      logResult(
        "Rating Validation",
        false,
        "Should have failed with invalid rating",
      );
    } catch (error) {
      if (error.errors && error.errors.rating) {
        logResult(
          "Rating Validation",
          true,
          "Rating validation works correctly",
        );
      } else {
