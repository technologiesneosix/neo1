...
export const getAllBlogs = async (req, res, next) => {
  try {
    const { page = 1, limit = 10, published, featured, category, author, tag, search, sortBy = 'createdAt', sortOrder = 'desc' } = req.query;
...
export const updateBlogStatus = async (req, res, next) => {
  try {
    ...
    return res.status(200).json(
      ApiResponse.success('Blog status updated successfully', blog)
    );
  } catch (error) {
    next(error);
  }
