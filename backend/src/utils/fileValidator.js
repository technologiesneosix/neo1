...
export const validateFileType = (mimetype, extension) => {
  ...
  const allAllowedExtensions = [...ALLOWED_IMAGE_EXTENSIONS, ...ALLOWED_VIDEO_EXTENSIONS, ...ALLOWED_DOCUMENT_EXTENSIONS];

  if (!allAllowedTypes.includes(mimetype)) {
    throw ApiError.badRequest(`Invalid file type. Allowed types: ${allAllowedTypes.join(', ')}`);
  }

  if (!allAllowedExtensions.includes(extension.toLowerCase())) {
    throw ApiError.badRequest(`Invalid file extension. Allowed extensions: ${allAllowedExtensions.join(', ')}`);
  }
};
